...
import random
from utils import make_composite_reward_gdpo
...
def train_gspo(
    ...
    if not os.path.exists(log_path):
        pd.DataFrame(columns=[
            "iteration", "step", "mean_reward", "mean_vina","mean_qed",
                "mean_sa","top1_reward", "top5_mean_reward",
    ...
    for it in range(1, iterations + 1):
        ...
        for step in range(1, M + 1):
            ...
            reward_items = make_composite_reward_gdpo(
                batch_smiles,
                vina_results,
                weights={'vina': 1, 'qed': 1, 'sa': 1},
            )
            ...
            mean_vina = float(np.mean([item['vina_raw'] for item in reward_items]))
            mean_qed = float(np.mean([item['qed_raw'] for item in reward_items]))
            mean_sa = float(np.mean([item['sa_raw'] for item in reward_items]))
            ...
            print(
                f"   meanR={mean_reward:.3f} | mean_vina =  {mean_vina:.3f} "
                f"  mean_qed = {mean_qed:.3f}|mean_sa = {mean_sa:.3f} \n"
            ...
            if mean_vina < best_vina:
                best_vina = mean_vina
                best_reward_state = copy.deepcopy(agent.state_dict())
                print(f"   New best reward: {best_vina:.3f}")
    # ===== 最终保存模型 =====

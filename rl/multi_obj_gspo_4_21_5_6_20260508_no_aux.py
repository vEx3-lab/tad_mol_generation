...
"""
GSPO 5_6 20260508 no-aux variant.

This version keeps the original GSPO mean-ratio policy objective, and applies
the 20260508 stability changes based on the latest training logs. It explicitly
removes elite auxiliary likelihood loss from all training calculations; the elite
buffer is retained only as a high-reward sample record.

1. Unique rollout batch refill:
   Sampling is still stochastic, but a peaked policy can repeatedly produce the
   same canonical molecule. We therefore keep sampling until we collect a target
   number of unique canonical SMILES, or until a bounded attempt budget is used.

2. True-token-KL beta control:
   The previous scalar KL proxy could remain small while the sampled-prefix
   token-distribution KL became large. KL beta is now adjusted using
   true_token_kl_mean from the diagnostics pass.

3. More exploratory sampling defaults:
   The default sampling temperature is raised and top-k is widened to reduce
   early distribution collapse while preserving the same sampler interface.
"""
...
def sample_unique_rollout_batch(
    model,
    vocab,
    batch_size=16,
    max_len=80,
    ...
    """
    Stochastically sample until the rollout batch contains enough unique
    canonical molecules, or until the bounded attempt budget is exhausted.
    """
    target_size = int(batch_size)
    max_attempts = max(target_size, target_size * int(max_attempt_multiplier))
    batch_token_ids = []
    batch_smiles = []
    seen = set()
    raw_sample_count = 0
    ...
    while len(batch_token_ids) < target_size and raw_sample_count < max_attempts:
        token_ids_list, smiles_list = sample_selfies_batch_from_generate_selfies(
            vocab=vocab,
            model=model,
            batch_size=1,
            max_len=max_len,
        ...
        if not token_ids_list or not smiles_list:
            continue

        token_ids = token_ids_list[0]
        smiles = smiles_list[0]
        canonical = canonicalize_smiles(smiles)
        key = canonical if canonical is not None else f"invalid::{raw_sample_count}::{smiles}"

        if canonical is not None and key in seen:
            duplicate_removed += 1
            continue
        if canonical is not None:
            seen.add(key)
    ...
    stats = {
        ...
        "dedup_count": len(batch_smiles),
        "duplicate_removed": duplicate_removed,
        "unique_sampling_attempts": raw_sample_count,
        "unique_sampling_target": target_size,
    }
    return batch_token_ids, batch_smiles, stats
...
def train_gspo_5_6(
    ...
    log_path = os.path.join(log_dir, f"gspo_5_6_no_aux_training_log_{timestamp}.csv")
    elite_path = os.path.join(log_dir, f"gspo_5_6_no_aux_elite_buffer_{timestamp}.csv")
    ...
    columns = [
        ...
        "raw_sample_count", "dedup_count", "duplicate_removed",
        "valid_count", "unique_smiles_rate", "mean_length",
        "seq_len_mean", "seq_len_max", "eos_included_rate",
        "vina_cached_count", "vina_success_count", "vina_failed_count",
        "policy_loss", "kl_loss", "gspo_loss", "total_loss", "ratio_mean",
        "ratio_sum_mean", "ratio_sum_std", "ratio_sum_min", "ratio_sum_max",
        "log_ratio_sum_mean", "true_token_kl_mean",
        "adv_mean", "adv_std", "adv_max_abs",
        "lr", "lr_before_update", "lr_after_update", "kl_beta", "target_kl", "kl_beta_adjusted",
        "kept_samples", "updates_applied", "grad_norm", "weight_decay",
        "clip_eps", "mu", "ratio_min", "ratio_max",
        "elite_size", "elite_added",
    ...
    for it in range(1, iterations + 1):
        print(f"\n{'=' * 70}")
        print(f" Iteration {it}/{iterations} (GSPO 5_6 no aux)")
        print(f"{'=' * 70}")
        ...
        for step in range(1, M + 1):
            ...
            if deduplicate_batch:
                batch_token_ids, batch_smiles, dedup_stats = sample_unique_rollout_batch(
                    model=old_agent,
                    vocab=vocab,
                    ...
                    top_k=top_k,
                    device=device,
                    max_attempt_multiplier=unique_attempt_multiplier,
                )
            else:
            ...
            if len(batch_token_ids) < 2:
                print("   Skip: not enough unique rollout samples")
                continue
            ...
            batch_smiles_valid = [x[2] for x in valid]
            ...
            for _ in range(mu):
                ...
                if kept_count == 0:
                    print("   Skip update: no GSPO samples")
                    continue
            ...
            if updates_applied > 0:
                kl_beta, kl_beta_adjusted = adjust_kl_beta(
                    kl_beta,
                    float(ratio_diagnostics["true_token_kl_mean"]),
                    target_kl,
                    kl_beta_min,
                    kl_beta_max,
                )
            ...
            row = {
                ...
                "duplicate_removed": dedup_stats["duplicate_removed"],
                ...
                "eos_included_rate": ratio_diagnostics["eos_included_rate"],
                ...
                "gspo_loss": float(gspo_loss.item()),
                "total_loss": float(total_loss.item()),
                "ratio_mean": float(r_mean.item()),
                "ratio_sum_mean": ratio_diagnostics["ratio_sum_mean"],
                "ratio_sum_std": ratio_diagnostics["ratio_sum_std"],
                "ratio_sum_min": ratio_diagnostics["ratio_sum_min"],
                "ratio_sum_max": ratio_diagnostics["ratio_sum_max"],
                "log_ratio_sum_mean": ratio_diagnostics["log_ratio_sum_mean"],
                "true_token_kl_mean": ratio_diagnostics["true_token_kl_mean"],
                "adv_mean": adv_stats["adv_mean"],
            ...
            print(
                ...
                f"(-{dedup_stats['duplicate_removed']}) | "

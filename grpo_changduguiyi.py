...
def compute_log_prob(model, seq_ids, pad_token_id=None):
    """
    计算一条完整序列的 log πθ(a_1:T)，并对序列长度归一化。

    参数:
        model: agent / old_agent
        seq_ids: 1D LongTensor [T+1]，包含 <SOS> ... <EOS>
        pad_token_id: 若有 PAD token，需要 mask 掉

    返回:
        avg_log_prob: scalar tensor（每步 log-prob 的平均值）
    """
    ...
    if pad_token_id is not None:
        ...
        seq_len = mask.sum(dim=-1).clamp(min=1)  # 避免除以 0

    # 序列 log-prob 平均
    avg_log_prob = token_log_probs.sum(dim=-1) / seq_len
    return avg_log_prob.squeeze(0)
...
#             continue
#
#         logp_new = compute_log_prob(agent, seq, pad_token_id=pad_token_id)
#         logp_old = compute_log_prob(old_agent, seq, pad_token_id=pad_token_id)
#
...
def compute_log_prob(
    ...
    seq_ids,
    pad_token_id=None,
    normalize=False   # ← 关键开关
):
    device = next(model.parameters()).device
    seq_ids = seq_ids.to(device)

    input_ids  = seq_ids[:-1].unsqueeze(0)
    target_ids = seq_ids[1:].unsqueeze(0)
    ...
    if pad_token_id is not None:
        ...
        length = token_log_probs.size(1)

    seq_log_prob = token_log_probs.sum()

    if normalize:
        seq_log_prob = seq_log_prob / length

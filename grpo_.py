...
def train_grpo(
    ...
    top_k=None,
    sync_old_every=1,
    log_dir="./feedback/logs",  # 新增参数，指定保存目录
):
    agent.to(device)
    ...
    log_path = os.path.join(log_dir, "grpo_training_log.csv")
    # 初始化 CSV 文件
    if not os.path.exists(log_path):
        df_init = pd.DataFrame(columns=[
            "epoch", "valid_sequences", "mean_reward", "policy_loss", "kl_loss"
        ])
        df_init.to_csv(log_path, index=False)
